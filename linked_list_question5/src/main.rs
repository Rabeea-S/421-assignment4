...
// And a Weak (weak Reference Counted) pointer to reference the previous node without owning it.
...
impl<T> List<T> {
    ...
    fn append(&mut self, data: T) {
        if let Some(ref mut last_node) = self.last {
            let new_node = Node::append(last_node, data);
            self.last = new_node;
